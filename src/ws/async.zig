...
pub fn Conn(comptime Handler: type) type {
    return struct {
        ...
        fn pong(self: *Self, payload: []const u8) !void {
            if (payload.len > 125) return error.ProtocolViolation;
        ...
        fn close(self: *Self, close_code: u16, payload: []const u8) !void {
            if (payload.len > 123) return error.ProtocolViolation;
...
test "async" {
    const Handler = struct {
        ...
        pub fn onRecv(self: *HandlerSelf, msg: Msg) void {
        ...
        pub fn sendZc(self: *HandlerSelf, buf: []const u8) !void {
            self.sent_frames += 1;
            testing.expectEqual(@as(usize, 6), buf.len) catch unreachable;
            testing.expectEqual(@as(u8, 0x8a), buf[0]) catch unreachable;
            testing.expectEqual(@as(u8, 0x80), buf[1]) catch unreachable;
