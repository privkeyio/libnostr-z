...
pub fn Client(comptime ReaderType: type, comptime WriterType: type) type {
    return struct {
        ...
        pub fn init(allocator: Allocator, rd: ReaderType, wr: WriterType) Self {
            return .{
                .reader = rd,
                .writer = wr,
                .sec_key = secKey(),
        ...
        const Header = struct {
            ...
            pub fn keyMatch(hdr: Header, k: []const u8) bool {
                return ascii.eqlIgnoreCase(hdr.key, k);
            }
            ...
            pub fn paramValue(hdr: Header, param: []const u8) ?[]const u8 {
                var it = std.mem.tokenizeAny(u8, hdr.value, ";= ");
                while (it.next()) |k| {
            ...
            pub fn match(hdr: Header, k: []const u8, v: []const u8) bool {
                return (ascii.eqlIgnoreCase(hdr.key, k) and
                    ascii.eqlIgnoreCase(hdr.value, v));
            }
        };
        ...
        const Response = struct {
            ...
            pub fn assertWebSocketUpgrade(rsp: *Response, sec_key_val: []const u8) !void {
                if (!rsp.isWebSocketUpgrade(sec_key_val)) return error.NotWebsocketUpgradeResponse;
            }

            pub fn isWebSocketUpgrade(rsp: *Response, sec_key_val: []const u8) bool {
                ...
                for (rsp.headers) |h| {
                    ...
                    if (h.keyMatch("sec-websocket-accept")) {
                        sec_accept_valid = isValidSecAccept(sec_key_val, h.value);
                    }
                }
        ...
        pub fn parseResponse(self: *Self) !Response {
            ...
            while (true) {
                ...
                const whitespace = " \t";
                const k = std.mem.trim(u8, header_line[0..index], whitespace);
                const v = std.mem.trim(u8, header_line[index + 1 ..], whitespace);

                try headers.append(Header{ .key = k, .value = v });
            }
...
fn clientInit(allocator: Allocator, rd: anytype, wr: anytype) Client(@TypeOf(rd), @TypeOf(wr)) {
    return Client(@TypeOf(rd), @TypeOf(wr)).init(allocator, rd, wr);
}

pub fn client(allocator: Allocator, rd: anytype, wr: anytype, uri: []const u8) !Options {
    var cs = clientInit(allocator, rd, wr);
    defer cs.deinit();
...
fn parsePath(uri: []const u8) []const u8 {
    var start = std.mem.indexOf(u8, uri, "//") orelse 0;
    start += if (start > 0) 2 else 0;
    const path_start = std.mem.indexOfPos(u8, uri, start, "/") orelse return "/";
...
pub const Rsp = struct {
    ...
    pub fn validate(self: Rsp, sec_key_val: []const u8) !void {
        if (!isValidSecAccept(sec_key_val, self.accept))
...
pub const Req = struct {
    ...
    pub fn parse(bytes: []const u8) !struct { Req, usize } {
        ...
        const is_get = mem.eql(u8, method_str, "GET");
        ...
        const is_http11 = mem.eql(u8, version_str, "HTTP/1.1");
...
pub fn requestAllocPrint(allocator: mem.Allocator, uri: []const u8, sec_key_val: []const u8) ![]const u8 {
    const host = parseHost(uri);
    return try std.fmt.allocPrint(allocator, request_format, .{ parsePath(uri), host, sec_key_val, host });
}

pub fn requestBufPrint(buf: []u8, uri: []const u8, sec_key_val: []const u8) ![]const u8 {
    const host = parseHost(uri);
    return try std.fmt.bufPrint(buf, request_format, .{ parsePath(uri), host, sec_key_val, host });
}

pub fn responseAllocPrint(allocator: mem.Allocator, accept_key: []const u8, options: Options) ![]const u8 {
    _ = options;
    return try std.fmt.allocPrint(allocator, response_format, .{accept_key});
}
