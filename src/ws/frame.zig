...
pub const Frame = struct {
    ...
    pub fn closeCode(self: Self) u16 {
        ...
        if (self.payload.len == 1) return 0;
    ...
    pub fn encode(self: Self, buf: []u8, close_code: u16) usize {
        ...
        const payload_len_usize = std.math.cast(usize, payload_len) orelse
            @panic("payload too large for platform");
    ...
    pub fn maskUnmask(mask_bytes: []const u8, buf: []u8) void {
        for (buf, 0..) |c, i|
    ...
    pub fn parse(data: []u8) !struct { Frame, usize } {
        ...
        const mask_bit: u1 = readBit(data[1], 0b1000_0000);
        ...
        return .{
            .{ .fin = fin, .rsv1 = rsv1, .mask = mask_bit, .opcode = opcode, .payload = payload },
            payload_end,
        };
    }
    ...
    fn readBit(byte: u8, comptime mask_val: u8) u1 {
        return if (byte & mask_val == 0) 0 else 1;
    }
...
test "parse close frame" {
    ...
    var data = frame_data ++ [_]u8{ 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff };
